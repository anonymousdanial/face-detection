...
DB_DIR = Path(os.getenv("FACES_DIR", "Faces"))  # Directory to store/load npy files
...
class FaceRecognizer:
    ...
    def _get_db_modified_time(self):
        ...
        try:
            database_file = DB_DIR / "face_encodings.npy"
    ...
    def _load_face_database(self):
        ...
        if not database_file.exists():
            ...
            np.save(database_file, {'face_ids': [], 'encodings': []})
            return
        ...
        try:
            ...
            if current_modified > self.db_last_modified:
                ...
                data = np.load(database_file, allow_pickle=True).item()
                self.known_face_encodings = data['encodings']
                self.known_face_names = data['face_ids']
        ...
        for i, face_encoding in enumerate(face_encodings):
            ...
            if len(self.known_face_encodings) == 0:
                ...
                if confidence >= CONFIDENCE_THRESHOLD:
                        # Check if recognized name is in db.customer_name
                        raw_name = self.known_face_names[best_match_index]
                        display_name = "Unknown"
                        try:
                            # raw_name is expected to be 'customer_<id>'
                            if isinstance(raw_name, str) and raw_name.startswith('customer_'):
                                ...
                                if row:
                                    # pyodbc.Row often supports attribute access by column name
